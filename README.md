# Hi, I'm Isul Alifajri 👋

I'm a Software Developer interested in building web applications and backend systems.

<img src="dev.png" alt="Developer" width="450">

## 🚀 About Me

- 💻 Full-Stack Developer working with PHP, Laravel, Yii2, Golang, CI3, VueJS, MySQL, PostgreSQL and SQLite
- 🛠️ Developing, maintaining, and improving web applications
- 🐛 Bug fixing, troubleshooting, and performance optimization
- ✨ Implementing new features and improving user experience
- 🌱 Always learning and exploring new technologies

## 🛠️ My Skills

![My Skills](https://skillicons.dev/icons?i=php,laravel,golang,vue,ai,python,jquery,nodejs,js,react,npm,vite,html,css,tailwind,bootstrap,mysql,postgres,sqlite,redis,firebase,supabase,docker,nginx,git,github,gitlab,bitbucket,vercel,ubuntu,vscode,postman,figma,discord&perline=8)

## 📫 Connect With Me

- GitHub: [isulalifajri](https://github.com/isulalifajri)
- LinkedIn: [isulalifajri](https://www.linkedin.com/in/isul-alifajri-128b6b245/)